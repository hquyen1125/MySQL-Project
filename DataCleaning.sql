...
-- These are the ones to delete where the row number is > 1 or 2 greater essentially
...
-- Create a new column and add those row numbers in. Then delete where row numbers are over 2, then delete the column
...
-- There are some null and empty rows
...
-- The Crypto has multiple different variations so standardize that
SELECT DISTINCT industry
FROM world_layoffs.layoffs_staging2
ORDER BY industry;
...
-- 3. Remove any columns and rows that is necessary to 
